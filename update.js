...
async function updateMembers(bot, clash) {
  ...
  for (const member of clanMembers) {
    ...
    if (player == null) { //новичок
      ...
      bot.channels.cache.get(bot.defaultChannel).send(`Приветствуем новичка ${member.name} в нашем клане!\nДобро пожаловать!`);
    }

    else if (player.hide) { //уже был в клане
      await player.set({ hide: false });
      await player.save();
      bot.channels.cache.get(bot.defaultChannel).send(`Старый знакомый ${member.name} вновь в нашем клане!\nИнтересно, что привело его вновь?`);
    }
  ...
  for (player of players) { //ищем ливнувших
    ...
    if (member == null && !player.hide) {
      ...
      await player.save();
      bot.channels.cache.get(bot.defaultChannel).send(`Игрок ${player.nickname} покинул клан.\nМы будем скучать (скорее всего нет)`);
    }
  }
}
...
async function updateWar(bot, clash) {
  ...
  if (curWar.state != 'warEnded' && curWar.opponent.tag != lastWar.opponent) {//если не заврешена, и текущая война не внесена - вносим и выходим
  ...
  for (const member of members) {
    ...
    if (attacks != null) {
      for (const attack of attacks) { //атаки
        ...
        let difficult = 1000 + (opTh - th)*200 - (opPos - pos)*50; //сложность атаки
